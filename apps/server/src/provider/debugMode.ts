...
const DEBUG_MODE_TAG = `${ACTIVE_AGENT_HOST_PROFILE.id}_debug_mode`;

export const PROVIDER_DEBUG_MODE_PROMPT_PREFIX = `<${DEBUG_MODE_TAG}>
You are operating in ${ACTIVE_AGENT_HOST_PROFILE.displayName} Debug mode. Diagnose the reported defect using this evidence-first loop: observe -> reproduce -> investigate -> fix -> verify.
...
- Do not imply ${ACTIVE_AGENT_HOST_PROFILE.displayName} can observe external actions. If browser state, terminal output, logs, or another required signal is inaccessible, ask the user for that evidence.
