...
import {
  COMPOSER_QUOTES,
  formatComposerQuote,
  pickComposerQuoteIndex,
} from "./chat/composerQuotes";
...
export default function ChatView({
  ...
  const appendLiveLatticeHostContext = useCallback(
    (text: string) =>
      appendLatticeHostContextToPrompt(text, isEmbed ? getLiveLatticeHostContext() : null),
    [isEmbed],
  );
  ...
  }, [activeThread?.id, hasThreadStarted, selectedProvider]);
  ...
  useEffect(() => {
    if (nextUserInputResponseReclaimAt === null) return;
    const timeoutId = window.setTimeout(
      () => setUserInputResponseClaimReferenceAt(new Date().toISOString()),
      Math.max(0, Date.parse(nextUserInputResponseReclaimAt) - Date.now()),
    );
  ...
  type ProviderFallbackProbe = {
    providerAtStart: ProviderKind;
    state: "failed" | "refreshing" | "settled";
  };
  const [providerFallbackProbes, setProviderFallbackProbes] = useState<
    ReadonlyMap<ThreadId, ProviderFallbackProbe>
  >(() => new Map());
  const providerFallbackProbesRef = useRef(new Map<ThreadId, ProviderFallbackProbe>());
  const storeProviderFallbackProbe = useCallback(
    (threadId: ThreadId, probe: ProviderFallbackProbe) => {
      providerFallbackProbesRef.current.set(threadId, probe);
      setProviderFallbackProbes(new Map(providerFallbackProbesRef.current));
    },
    [],
  );
  ...
  useEffect(() => {
    ...
    storeProviderFallbackProbe(activeThread.id, probe);
    void refreshProviderStatusesRef.current({ silent: true }).then((statuses) => {
      storeProviderFallbackProbe(activeThread.id, {
        providerAtStart: probe.providerAtStart,
        state: statuses ? "settled" : "failed",
      });
    });
  }, [activeThread, hasThreadStarted, selectedProvider, storeProviderFallbackProbe]);
  const providerFallbackProbe = activeThread ? providerFallbackProbes.get(activeThread.id) : null;
  ...
  const consumeDispatchedLatticeHostSelection = useCallback(
    (prompt: string) => {
      if (
        !isEmbed ||
        !promptContainsLiveLatticeHostSelection(prompt, getLiveLatticeHostContext())
      ) {
        return;
      }
  ...
  useLayoutEffect(() => {
    ...
    const syncComposerFooterLayout = () => {
      ...
      if (footerRow) {
        ...
        const actions = footerRow.querySelector<HTMLElement>(
  ...
  const armLocalDispatchAckFallback = useCallback((threadIdForSend: ThreadId) => {
    ...
    localDispatchAckFallbackTimeoutRef.current = window.setTimeout(() => {
      ...
      if (serverAcknowledgedLocalDispatchRef.current) return;
      setLocalDispatch((current) =>
        current &&
  ...
  const onSend = async (
    ...
    return (async () => {
      ...
      if (worktreeSetupResolution) setWorktreeSetupPendingAction(null);
      ...
      beginLocalDispatch({
        ...
        ...(baseBranchForWorktree
          ? {
              worktreeSetupStepId: "create-branch",
      ...
      await (async () => {
        ...
        const consumeWorktreeSetupResolution = async () => {
          ...
          if (action === null || switchedToLocalCheckout) return;
          if (action === "cancel") throw new WorktreeSetupCancelledError();
        ...
        if (!setupCancelled) failLocalDispatchWorktreeSetup();
        if (!turnStartSucceeded) clearPendingTurnDispatch(threadIdForSend);
  ...
  const renderComposerLeadingControls = (options: { iconOnly: boolean }) => (
    <>
      <ComposerExtrasMenu
        ...
        {...(isEmbed ? { triggerClassName: "!size-8" } : {})}
      />
                        ...
                        showPlanFollowUpPrompt ? (
                          ...
                          <>
                            ...
                            <Button
                              ...
                              className={cn(
                                "rounded-full",
                                isEmbed ? "!size-6" : "size-7 sm:size-7",
                              )}
                              disabled={
                                isSendBusy ||
                                isConnecting ||
