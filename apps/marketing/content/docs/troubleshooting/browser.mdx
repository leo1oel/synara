...
## Page is clipped or leaves empty space

An agent can use `browser_resize` to test a fixed page size. Resizing or reopening the browser panel, or switching between its floating and expanded views, restores the page to the panel's available space without reloading it. Moving a panel without changing its size preserves the requested viewport.
