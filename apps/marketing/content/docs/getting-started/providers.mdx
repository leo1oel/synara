...
The activity switch allows background checks and new turns; it does not install the runtime or sign you in. Check the setup status beneath each provider, open its **Setup guide** for installation and authentication instructions, then select **Refresh status** after completing setup.

Some providers, including OpenCode, do not expose an authentication check to Synara. **Installed · sign-in not verified** means the runtime was detected, but you still need to configure and verify your model-provider credentials in that runtime.
