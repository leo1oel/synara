...
| 10  | Generic helpers                                                                                                                        | `utf8Truncation.ts` (string-length clamping), `computerGeometry.ts`, `screenshotFrames.ts` (frame→desktop mapping), `waitForWindow.ts`/`waitForControl.ts`                                                                                                                                                                                  | **package module** if the package includes a backend contract; otherwise they ride with `ComputerBackend.ts`                                                                                                                                                                                                   | Low                                                                                                                                                                            |
